...
import { Committee, CommitteeType, COMMITTEE_TYPES } from './CommitteesAdmin';

interface CommitteesListProps {
  committees: Committee[];
  onCommitteesChange: (committees: Committee[]) => void;
  onManage: (committee: Committee) => void;
}
...
export const CommitteesList: React.FC<CommitteesListProps> = ({
  committees,
  onCommitteesChange,
  onManage,
}) => {
  ...
  const [formType, setFormType] = useState<CommitteeType | ''>('');
  ...
  const handleSave = () => {
    ...
    if (modalMode === 'add') {
      const newCommittee: Committee = {
        id: `c_${Date.now()}`,
        name: formName.trim(),
        type: formType as CommitteeType,
      };
      onCommitteesChange([...committees, newCommittee]);
    } else if (modalMode === 'edit' && editingCommittee) {
      onCommitteesChange(
        committees.map(c =>
          c.id === editingCommittee.id
            ? { ...c, name: formName.trim(), type: formType as CommitteeType }
            : c
        )
      );
    }
  ...
  return (
    <div className="min-h-full">
      ...
      <div className="flex items-center justify-between mb-5">
        <h1 className="text-xl font-semibold text-gray-800">Committees</h1>
        <button
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 text-left">
              <th className="px-4 py-3 font-semibold text-gray-600 uppercase text-xs tracking-wide w-1/2">Committee</th>
              <th className="px-4 py-3 font-semibold text-gray-600 uppercase text-xs tracking-wide w-1/4">Type</th>
            ...
            {paginated.length === 0 ? (
              <tr>
                <td colSpan={3} className="px-4 py-8 text-center text-gray-400">
                  {searchQuery ? 'No committees match your search.' : 'No records available'}
                </td>
              </tr>
            ) : (
              paginated.map(committee => (
                <tr key={committee.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-4 py-3 font-medium text-gray-800">{committee.name}</td>
                  <td className="px-4 py-3 text-sm text-gray-700">{committee.type}</td>
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-center gap-3">
                      <button
                        onClick={() => onManage(committee)}
                      ...
                      <div className="relative">
                        <button
                          onClick={() => setOpenMenuId(openMenuId === committee.id ? null : committee.id)}
                          className="text-gray-400 hover:text-gray-700 p-1 rounded transition-colors"
                        >
                          <MoreHorizontal className="w-4 h-4" />
                        </button>
                        {openMenuId === committee.id && (
                          <div className="absolute right-0 top-7 z-20 bg-white border border-gray-200 rounded shadow-lg py-1 w-36">
                            ...
                            <button
                              onClick={() => {
                                setDeleteConfirm({ id: committee.id, name: committee.name });
                                setOpenMenuId(null);
                              }}
          ...
          <div className="bg-white w-full max-w-lg h-full shadow-2xl flex flex-col">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
              <h2 className="text-lg font-semibold text-gray-800">
                {modalMode === 'add' ? 'Add Committee' : 'Edit Committee'}
              </h2>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-6 space-y-5">
              ...
              <div>
                ...
                <select
                  value={formType}
                  onChange={e => setFormType(e.target.value as CommitteeType | '')}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-lg shadow-2xl p-6 w-full max-w-sm mx-4">
            <div className="flex items-center gap-3 mb-3">
              ...
              <h3 className="text-base font-semibold text-gray-800">Delete Committee</h3>
            </div>
