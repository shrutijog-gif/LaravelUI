...
import { Settings, X, Plus, Eye, EyeOff, Trash2, GripVertical, AlignLeft, Layers, Home, Pencil, CheckCircle2, AlertCircle } from 'lucide-react';
...
const COMMITTEE_TABS_PREFIX = 'college_committee_tabs_';

export const getCommitteeTabs = (committeeId: string): CommitteeTab[] => {
  try {
    const raw = localStorage.getItem(`${COMMITTEE_TABS_PREFIX}${committeeId}`);
    if (raw) {
      const parsed = JSON.parse(raw);
      if (Array.isArray(parsed) && parsed.length > 0) return parsed;
    }
  } catch (e) {
    console.error('Error reading committee tabs', e);
  }
  return defaultTabs();
};

export const saveCommitteeTabs = (committeeId: string, tabs: CommitteeTab[]): void => {
  try {
    localStorage.setItem(`${COMMITTEE_TABS_PREFIX}${committeeId}`, JSON.stringify(tabs));
  } catch (e) {
    console.error('Error saving committee tabs', e);
  }
};

export const syncCommitteeTabsToAll = (sourceTabs: CommitteeTab[]): number => {
  try {
    const committeeIds = ['c1', 'c2', 'c3', 'c4', 'c5'];
    committeeIds.forEach(id => {
      const existingTabs = getCommitteeTabs(id);
      const merged: CommitteeTab[] = sourceTabs.map(srcTab => {
      ...
      saveCommitteeTabs(id, merged);
    });
    window.dispatchEvent(new CustomEvent('committee-tabs-synced'));
    return committeeIds.length;
  } catch (e) {
    console.error('Error syncing committee tabs to all', e);
    return 0;
  }
};
...
const TabManagerModal: React.FC<{
  ...
  const handleConfirmSyncAll = () => {
    const count = syncCommitteeTabsToAll(localTabs);
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg mx-4 flex flex-col" style={{ maxHeight: '85vh' }}>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-gray-100">
          <button
            ...
            title="Apply this section structure to all committees"
          >
              ...
              {syncedCount !== null ? (
                <div className="text-center py-4">
                  ...
                  <p className="text-xs text-gray-600">
                    Successfully applied section structure across all {syncedCount} committees.
                  </p>
                </div>
              ) : (
                <>
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div>
                      <h3 className="text-base font-bold text-gray-900">Apply to All Committees?</h3>
                      <p className="text-xs text-gray-500">Sync section structure</p>
                    </div>
                  </div>
                  <p className="text-xs text-gray-600 leading-relaxed mb-5">
                    This will sync the section list, order, names, and visibility across <strong>all committees</strong>. Each committee's own written content and members will remain safe.
                  </p>
...
export const CommitteeContentManager: React.FC<CommitteeContentManagerProps> = ({ committee, onBack }) => {
  const [tabs, setTabs] = useState<CommitteeTab[]>(() => getCommitteeTabs(committee.id));
  ...
  useEffect(() => {
    const handleGlobalSync = () => {
      setTabs(getCommitteeTabs(committee.id));
    };
    window.addEventListener('committee-tabs-synced', handleGlobalSync);
    return () => window.removeEventListener('committee-tabs-synced', handleGlobalSync);
  }, [committee.id]);
  ...
  const updateTabContent = (tabId: string, content: string) => {
    setTabs(prev => {
      ...
      saveCommitteeTabs(committee.id, next);
      return next;
    });
  };
  ...
  const handleTabsChange = (updated: CommitteeTab[]) => {
    setTabs(updated);
    saveCommitteeTabs(committee.id, updated);
